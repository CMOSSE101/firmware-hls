...
#pragma HLS unroll
    ...
    if (validin_) {
      ...
      constexpr auto nbits_all = LAYER < trklet::N_LAYER ? nbitsallstubs[LAYER] : nbitsallstubs[DISK];
      
      // number of bits used to distinguish between VMs within a module
      constexpr auto nbits_vmme = LAYER < trklet::N_LAYER ? nbits_vmmeall[LAYER] : nbits_vmmeall[DISK];
      ...
      */
      ...
      /*
      ap_uint<1> usefirstMinus = instubdata.getEntries(bx, ivmMinus*nbins + zfirst) != 0;//nstubfirstMinus != 0;
      ap_uint<1> usesecondMinus = useSecond && (instubdata.getEntries(bx, ivmMinus*nbins + zfirst) != 0);//(useSecond && nstublastMinus != 0);
      ap_uint<1> usefirstPlus = ivmPlus != ivmMinus && (instubdata.getEntries(bx, ivmMinus*nbins + zfirst) != 0);//ivmPlus != ivmMinus && nstubfirstPlus != 0;
      ap_uint<1> usesecondPlus = ivmPlus != ivmMinus && (useSecond && (instubdata.getEntries(bx, ivmMinus*nbins + zfirst) != 0));//ivmPlus != ivmMinus && (useSecond && nstublastPlus != 0);
      */
      ...
      if (nstubs!=0) {
#ifdef DEBUG
        std::cout << "Sending " << allproj.raw() << " to PR buffer" << std::endl;
        ...
        std::cout << "useSecond=" << useSecond << std::endl;
        ...
        std::cout << instubdata.getEntries(bx, ivmMinus*nbins + zfirst) << "\t"
                  << instubdata.getEntries(bx, ivmMinus*nbins + zfirst) << "\t"
                  << instubdata.getEntries(bx, ivmMinus*nbins + zfirst) << "\t"
                  << instubdata.getEntries(bx, ivmMinus*nbins + zfirst) << "\t" << std::endl;
