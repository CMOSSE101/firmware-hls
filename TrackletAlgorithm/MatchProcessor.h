...
#pragma HLS ARRAY_PARTITION variable=rbinLUT complete dim=0
  ...
  //readRbin_LUT<LAYER,nRbinBits,(1<<TrackletProjection<PROJTYPE>::kTProjRZSize)>(rbinLUT);

  //ap_uint<4> nvmstubs[2*(1<<3)][2*(1<<3)]; 
...
#pragma HLS unroll
    ...
    if (validin_) {
      ...
      */
      ...
      if (usefirstPlus || usesecondPlus || usefirstMinus || usesecondMinus) {//nstubs!=0) {
#ifdef DEBUG
      std::cout << "Checking proj=" << allproj.raw() << " with zfrist=" << zbin.range(zbin.length()-1, 1) << std::endl;
      std::cout << bx << "," << ivmMinus << "*" <<nbins << "+" << zbin.range(zbin.length()-1, 1) << " = " << (ivmMinus*nbins + zbin.range(zbin.length()-1, 1)) << std::endl;
      std::cout << bx << "," << ivmMinus << "*" <<nbins << "+" << zbin.range(zbin.length()-1, 1) + 1 << " = " << (ivmMinus*nbins + zbin.range(zbin.length()-1, 1) + 1) << std::endl;
      std::cout << bx << "," << ivmPlus << "*" <<nbins << "+" << zbin.range(zbin.length()-1, 1) << " = " << (ivmPlus*nbins + zbin.range(zbin.length()-1, 1)) << std::endl;
      std::cout << bx << "," << ivmPlus << "*" <<nbins << "+" << zbin.range(zbin.length()-1, 1) + 1 << " = " << (ivmPlus*nbins + zbin.range(zbin.length()-1, 1) + 1) << std::endl;
      std::cout << instubdata.getEntries(bx, ivmMinus*nbins + zbin.range(zbin.length()-1, 1)) << "\t"
                << instubdata.getEntries(bx, ivmMinus*nbins + zbin.range(zbin.length()-1, 1) + 1) << "\t"
                << instubdata.getEntries(bx, ivmPlus*nbins + zbin.range(zbin.length()-1, 1)) << "\t"
                << instubdata.getEntries(bx, ivmPlus*nbins + zbin.range(zbin.length()-1, 1) + 1) << "\t" << std::endl;
      std::cout << "phi=" << iphi << "\tivmMinus=" << ivmMinus << "\t" << "ivmPlus=" << ivmPlus << "\t" << "useSecond=" << useSecond << std::endl;
      std::cout << (instubdata.getEntries(bx, ivmMinus*nbins + zbin.range(zbin.length()-1, 1)) != 0) << "\t"
                << (useSecond && (instubdata.getEntries(bx, ivmMinus*nbins + zbin.range(zbin.length()-1, 1) + 1) != 0)) << "\t"
                << (ivmPlus!=ivmMinus && (instubdata.getEntries(bx, ivmPlus*nbins + zbin.range(zbin.length()-1, 1)) != 0)) << "\t"
                << (ivmPlus!=ivmMinus && (useSecond && (instubdata.getEntries(bx, ivmPlus*nbins + zbin.range(zbin.length()-1, 1) + 1) != 0))) << "\t" << std::endl;
#endif
#ifdef DEBUG
      std::cout << "proj=" << allproj.raw() << std::endl;
      ...
      std::cout << "nstubs=\t"
        ...
        std::cout << "instub entries[rfirst][ivmMinus]=" << nvmstubs[rfirst][ivmMinus] << std::endl;
        ...
        std::cout << "projdata_=" << projdata_.raw() << std::endl;
        std::cout << "not zero\t" << (instubdata.getEntries(bx, ivmMinus*nbins + zfirst) != 0) << "\t"//nstubfirstMinus != 0;
                  << (instubdata.getEntries(bx, ivmMinus*nbins + zfirst + 1) != 0) << "\t"//(useSecond && nstublastMinus != 0);
                  << (instubdata.getEntries(bx, ivmPlus*nbins + zfirst) != 0) << "\t"//ivmPlus != ivmMinus && nstubfirstPlus != 0;
                  << (instubdata.getEntries(bx, ivmPlus*nbins + zfirst + 1) != 0) << std::endl;//ivmPlus != ivmMinus && (useSecond && nstublastPlus != 0);
        std::cout << (nstubfirstMinus != 0) << "\t"
                  << (nstublastMinus != 0) << "\t"
                  << (nstubfirstPlus != 0) << "\t"
                  << (nstublastPlus != 0) << std::endl;
#endif
